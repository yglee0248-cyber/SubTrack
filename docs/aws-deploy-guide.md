...
2. database/02_seed_category.sql
```

`database/03_*` 이후 파일은 기존 로컬 DB를 유지하면서 변경분을 반영하기 위한 수동 마이그레이션 용도로 본다. 새 RDS에는 먼저 `01_schema.sql`과 `02_seed_category.sql` 기준으로 초기화한다.
...
3. RDS에 database/01_schema.sql, database/02_seed_category.sql 적용 여부 확인
