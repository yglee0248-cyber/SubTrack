...
```env
PROD_DB_URL=jdbc:mysql://<rds-endpoint>:3306/subtrack?serverTimezone=Asia/Seoul&useUnicode=true&characterEncoding=utf8&connectionCollation=utf8mb4_unicode_ci&characterSetResults=utf8mb4
PROD_DB_USERNAME=<rds-app-username>
PROD_DB_PASSWORD=<rds-app-password>
PROD_JWT_SECRET=<jwt-secret>
PROD_JWT_ACCESS_TOKEN_EXPIRATION_MS=<jwt-access-token-expiration-ms>
PROD_CORS_ALLOWED_ORIGINS=https://<frontend-domain>
PROD_EXCHANGE_RATE_BASE_URL=<exchange-rate-api-base-url>
```

주의:

- DB 비밀번호, JWT secret, SSH private key, AWS secret access key는 반드시 Secrets에 둔다.
- `PROD_DB_URL`에는 RDS endpoint가 포함되므로 팀 정책에 따라 Secrets로 관리하는 편이 안전하다.
...
EXCHANGE_RATE_BASE_URL=<exchange-rate-api-base-url>
```
