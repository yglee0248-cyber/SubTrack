...
- Spring Scheduler는 P1 알림 기능 후보
- JUnit

### Push

- OneSignal은 P1 이후 알림 기능 후보
...
- 현재 기준 upcoming, due today는 현재 `status = ACTIVE`이고 `deleted_at IS NULL`인 구독만 사용
- `deleted_at IS NOT NULL`인 구독은 목록과 대시보드에서 전체 제외
...
목록 조회와 대시보드에서는 `deleted_at IS NULL`인 항목만 사용합니다.
...
알림 기능은 현재 MVP 화면과 API에는 포함하지 않고, P1 이후 후보로 검토합니다.
